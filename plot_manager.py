...
class PlotManager:
        ...
        if tab_name in app.tabs:
            ...
            for w in tab.winfo_children():
                 if w.winfo_exists():
                     ...
                     elif tab_name != 'Waveform' or not (hasattr(app, 'section_editor') and app.section_editor and (w == app.section_editor or w == app.section_editor.master)):
            ...
            if not has_other_content and tab_name != 'Waveform':
                ...
                if tab_name not in app.plot_widgets: app.plot_widgets[tab_name] = {}
        ...
        for name in app.tab_names:
            if name == 'Waveform':
                 if name in app.plot_widgets and 'placeholder' in app.plot_widgets[name]:
                     if app.plot_widgets[name]['placeholder'].winfo_exists(): app.plot_widgets[name]['placeholder'].destroy()
            ...
            if map_key and not app.analysis_vars[map_key].get(): is_analysis_selected = False; msg = "Analysis not selected."
            elif name == 'Band Analysis' and not (app.analysis_vars['stereo'].get() or app.analysis_vars['hpss'].get()): is_analysis_selected = False; msg = "Requires Stereo or HPSS Analysis."
            files_ok = (app.mode.get() == "single" and app.file_path.get(1)) or (app.mode.get() == "compare" and app.file_path.get(1) and app.file_path.get(2))
            if not is_analysis_selected or not files_ok: self.add_placeholder_label(name, msg if not is_analysis_selected else "Select file(s)...")
            else: self.add_placeholder_label(name, f"Ready for {name} analysis...")
        ...
        for name, widgets in app.plot_widgets.items():
            if name in app.tabs:
                ...
                if tab.winfo_exists():
                    ...
                    if name == 'Waveform' and keep_editor and hasattr(app, 'section_editor') and app.section_editor:
                        if app.section_editor.master and app.section_editor.master.winfo_exists(): widgets_to_keep.append(app.section_editor.master)
                         ...
                         if widget:
                             try:
                                 if isinstance(widget, (FigureCanvasTkAgg)):
                                     if widget.get_tk_widget() and widget.get_tk_widget().master and widget.get_tk_widget().master.winfo_exists(): widget.get_tk_widget().master.destroy()
            ...
            if name == 'Waveform' and hasattr(app, 'waveform_summary_label') and app.waveform_summary_label:
                 try: app.waveform_summary_label.destroy() if app.waveform_summary_label.winfo_exists() else None
                 except Exception: pass
                 app.waveform_summary_label = None
        # Clear editor content only if not keeping it
        if not keep_editor and app.section_editor: app.section_editor.clear()
        app.plot_widgets = {name: {} for name in app.tab_names}
        app.waveform_summary_label = None; app.playhead_line = None
        # Clear HMM results if they exist (handled in main_app clear now)
        # if app.track_data.get(1):
    ...
    def embed_plot(self, fig, tab_name):
        ...
        if tab_name not in app.tabs: print(f"Error: Tab '{tab_name}' not found."); plt.close(fig); return
        tab=app.tabs[tab_name];
        print(f"DEBUG PlotManager: Clearing plot widgets in tab {tab_name}")
        widgets_to_keep = []
        if tab_name == 'Waveform' and hasattr(app, 'section_editor') and app.section_editor:
             if app.section_editor.master and app.section_editor.master.winfo_exists():
                 widgets_to_keep.append(app.section_editor.master) # Keep the outer frame
        for widget in list(tab.winfo_children()):
            if widget not in widgets_to_keep:
                try: widget.destroy();
                except Exception: pass
        if tab_name in app.plot_widgets: app.plot_widgets[tab_name] = {}
        if tab_name == 'Waveform': app.waveform_summary_label = None; app.playhead_line = None
        try:
            plot_frame = ttk.Frame(tab); plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
            print(f"DEBUG PlotManager: Creating canvas for {tab_name}"); canvas = FigureCanvasTkAgg(fig, master=plot_frame); canvas_widget = canvas.get_tk_widget()
            print(f"DEBUG PlotManager: Creating toolbar for {tab_name}"); toolbar = NavigationToolbar2Tk(canvas, plot_frame); toolbar.update()
            if tab_name == 'Waveform':
                print(f"DEBUG PlotManager: Creating waveform summary label"); app.waveform_summary_label = ttk.Label(plot_frame, text="Structure: Calculating...", padding=(5, 2), anchor=tk.W, justify=tk.LEFT, wraplength=800); app.waveform_summary_label.pack(side=tk.BOTTOM, fill=tk.X, pady=(5,0)); print(f"DEBUG PlotManager: Waveform summary label packed")
                axes_list = fig.get_axes()
                if axes_list:
                    ax = axes_list[0]; app.playhead_line, = ax.plot([0, 0], ax.get_ylim(), color='white', lw=1.0, alpha=0.8, visible=False, zorder=10); print(f"DEBUG PlotManager: Initial playhead line created (visible=False)")
                else: print("Warning: Could not get axes from figure for playhead line."); app.playhead_line = None
                print(f"DEBUG PlotManager: Connecting click event for Waveform canvas"); canvas.mpl_connect('button_press_event', app._on_waveform_click) # Call main app's handler
            print(f"DEBUG PlotManager: Packing toolbar for {tab_name}"); toolbar.pack(side=tk.BOTTOM, fill=tk.X)
            print(f"DEBUG PlotManager: Packing canvas for {tab_name}"); canvas_widget.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
            app.plot_widgets[tab_name]={'canvas':canvas,'toolbar':toolbar,'figure':fig}; print(f"DEBUG PlotManager: Widgets stored for {tab_name}")
        ...
        finally: plt.close(fig); print(f"DEBUG PlotManager: _embed_plot finished for tab: {tab_name}")
    ...
    def display_analysis_results(self):
        ...
        if t1_data_source:
            ...
            if app.show_hmm_var.get() and 'hmm_semantic_labels' in t1_data_source and t1_data_source['hmm_semantic_labels']:
                print("DEBUG PlotManager: Using HMM labels and colors for display.")
                display_data1['semantic_labels'] = t1_data_source.get('hmm_semantic_labels', [])
                display_data1['label_colors'] = t1_data_source.get('hmm_label_colors', [])
                display_data1['section_starts'] = t1_data_source.get('hmm_section_starts', [])
                # Disable pre-cleanup toggle when showing HMM results
                if hasattr(app, 'toggle_labels_button'): app.toggle_labels_button.config(state=tk.DISABLED)
                if hasattr(app, 'show_pre_cleanup_labels_var'): app.show_pre_cleanup_labels_var.set(False)
            else:
                print("DEBUG PlotManager: Using Original analysis labels and colors for display.")
        ...
        for tab_name, s_func, c_func, p_func_comp, req_keys, s_func_kwargs in plot_configs:
            if tab_name not in app.tabs: continue
            ...
            if core_analysis and core_analysis in app.analysis_vars and not app.analysis_vars[core_analysis].get(): analysis_enabled = False
            elif tab_name == 'Band Analysis' and not (app.analysis_vars['stereo'].get() or app.analysis_vars['hpss'].get()): analysis_enabled = False; print(f" Skipping {tab_name}: Needs Stereo or HPSS Analysis.")
            elif tab_name in ['Energy/Balance', 'Timbre/Texture'] and not app.analysis_vars['chroma'].get(): analysis_enabled = False; print(f" Skipping {tab_name}: Needs Chroma/Labeling Analysis for features.")

            # Use display_data1/2 which contain the selected labels/colors/starts
            data_available = bool(display_data1) if mode == "single" else (bool(display_data1) or bool(display_data2))

            if analysis_enabled and data_available:
                print(f" PlotManager: Plotting {tab_name} using {display_mode} data..."); fig = None
                try:
                    ...
                    if mode=="single":
                        ...
                        if s_func == ap.create_band_analysis_plot: fig = s_func(plot_data1, app.track_names[1])
                        elif s_func == ap.create_single_track_plot: fig = s_func(plot_data1, p_func_comp, tab_name, app.track_names[1], **s_func_kwargs)
                    else: # Compare mode (HMM comparison not implemented, uses original)
                         plot_data1_comp = t1_data_source.copy() if t1_data_source else {} # Use original for T1 in compare
                         plot_data2_comp = t2_data_source.copy() if t2_data_source else {} # Use original for T2
                         if c_func == ap.create_band_comparison_plot: fig = c_func(plot_data1_comp, plot_data2_comp, app.track_names[1], app.track_names[2])
                         elif c_func is not None: fig = c_func(plot_data1_comp, plot_data2_comp, p_func_comp, f"{tab_name} Comparison", app.track_names[1], app.track_names[2])
                         else: print(f" Skipping comparison for {tab_name}."); self.add_placeholder_label(tab_name, "Comparison not available.")

                    if fig:
                        self.embed_plot(fig, tab_name) # Call self.embed_plot
                        # Update summary label (only for original view on waveform tab)
                        if tab_name == 'Waveform' and mode == 'single' and app.waveform_summary_label is not None:
                             if not app.show_hmm_var.get(): # Only show summary for original view
                                 ...
                                 if labels_for_summary and features_for_summary:
                                     try: summary_string = "Structure (T1): " + ap.generate_structure_summary(labels_for_summary, features_for_summary)
                                 ...
                                 app.waveform_summary_label.config(text=summary_string); app.master.update_idletasks()
                                 wrap_w = app.tabs[tab_name].winfo_width() - 20; app.waveform_summary_label.config(wraplength=max(100, wrap_w))
                             else: # Clear summary for HMM view
                                  app.waveform_summary_label.config(text="Structure Summary (N/A for HMM view)")

                    elif analysis_enabled: self.add_placeholder_label(tab_name, "Plot skipped (figure generation failed).")
                except Exception as plot_e: print(f"--- Error plot {tab_name} ---"); traceback.print_exc(); self.add_placeholder_label(tab_name, f"Error:\n{plot_e}"); plt.close(fig) if fig else None
            elif not data_available: print(f" Skipping {tab_name} (analysis data missing)."); self.add_placeholder_label(tab_name, "Analysis data missing.")
            else: print(f" Skipping {tab_name} (analysis not selected / dependencies missing)."); self.add_placeholder_label(tab_name, "Analysis not selected / dependencies missing.")

        # Section editor always shows original analysis data for editing
        if mode == "single" and app.section_editor and t1_data_source:
             print("DEBUG PlotManager: Populating section editor with original analysis data.")
             app.section_editor.populate(t1_data_source)
