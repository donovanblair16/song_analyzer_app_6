...
def prepare_multi_feature_data(all_track_data_with_filenames, feature_keys, cleaning_settings):
    ...
    Args:
        all_track_data_with_filenames (list): List of tuples, where each tuple
            contains (filename, data_dict).
        feature_keys (list): List of feature names (strings) to use, e.g.,
            ['spectral_contrast_mean', 'chroma_std'].
        cleaning_settings (dict): Dictionary with cleaning parameters. Keys include:
    ...
    Returns:
        ...
        1.  **X_scaled** (np.ndarray): Scaled feature array (not weighted yet).
        2.  **sequence_lengths** (list): List of sequence lengths for HMM input.
        ...
        7.  **raw_averages** (dict): Raw feature averages by label before scaling.
