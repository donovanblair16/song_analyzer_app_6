...
if __name__ == "__main__":
    ...
    cleaning_settings = user_config["cleaning_settings"]
    use_transition_prior = user_config[
    ...
    if cleaning_settings.get("enable_outlier_removal"):
        cleaning_flags.append("out")
    if cleaning_settings.get("enable_short_section_removal"):
        cleaning_flags.append("sh")
    if cleaning_settings.get("enable_consistency_filtering"):
        cleaning_flags.append("con")
    cleaning_str = "_".join(cleaning_flags) if cleaning_flags else "noclean"
    # Add flag if prior is NOT used
    prior_flag = "" if use_transition_prior else "_noprior"

    base_filename = f"gmmhmm_{feature_str}_{mixture_str}_{cleaning_str}{prior_flag}"  # Add prior flag to name
    ...
    print(f"Use Transition Prior: {use_transition_prior}")  # <<< Print the setting
    print(f"Data Cleaning Settings:")
    print(
        f"  - Outlier Removal: {'ENABLED (z=' + str(cleaning_settings.get('outlier_z_threshold', 'N/A')) + ')' if cleaning_settings.get('enable_outlier_removal') else 'DISABLED'}"
    )
    print(
        f"  - Short Section Removal: {'ENABLED (min_bars=' + str(cleaning_settings.get('short_section_min_bars', 'N/A')) + ')' if cleaning_settings.get('enable_short_section_removal') else 'DISABLED'}"
    )
    print(
        f"  - Consistency Filtering: {'ENABLED' if cleaning_settings.get('enable_consistency_filtering') else 'DISABLED'}"
    )
    ...
    # 2. Load "Perfect" Analysis Data
    print("[DEBUG] Loading perfect analyses...")
    all_data_with_filenames = load_perfect_analyses(PERFECT_FOLDER_PATH)
    if not all_data_with_filenames:
        print("ERROR: No valid 'Perfect' track data found. Cannot train HMM. Exiting.")
        sys.exit(1)
    print(f"[DEBUG] Loaded data for {len(all_data_with_filenames)} tracks.")

    # 3. Prepare Multi-Feature Data
    print("[DEBUG] Preparing multi-feature data...")
    prep_result = prepare_multi_feature_data(
        all_data_with_filenames, feature_keys, cleaning_settings
    )
    ...
    transition_matrix_prior = None  # Default to None
    # <<< Only load if user selected the option >>>
            ...
            try:
                ...
                if loaded_prior.shape == (n_unique_states, n_unique_states):
                    if np.allclose(np.sum(loaded_prior, axis=1), 1.0, atol=1e-6):
                        transition_matrix_prior = loaded_prior  # Assign if valid
    ...
    # <<< Pass the potentially loaded prior (or None) >>>
    train_result = train_gmmhmm(
        ...
        hmm_n_mixtures,
        hmm_min_covar,
        transmat_prior=transition_matrix_prior,  # Pass the variable here
    )
        ...
        try:
            ...
            aux_data = {
                ...
                "hmm_params": {
                    ...
                    "used_transition_prior": (
                        transition_matrix_prior is not None
                    ),  # Record if prior was actually used
                },
                "cleaning_settings_used": cleaning_settings,
